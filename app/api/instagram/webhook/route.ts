...
function isValidSignature(rawBody: string, signatureHeader: string | null): boolean {
  if (process.env.INSTAGRAM_ACCESS_TOKEN === "MOCK_TOKEN") return true
...
async function verifyFollowStatus(igScopedId: string, pageAccessToken: string): Promise<{ follows: boolean | null; error?: 'auth' | 'transient' }> {
  if (pageAccessToken === "MOCK_TOKEN" || process.env.INSTAGRAM_ACCESS_TOKEN === "MOCK_TOKEN") {
    console.log(`[ig-api-mock] Mocking verifyFollowStatus for ${igScopedId}`)
    return { follows: true, error: undefined }
  }
  try {
